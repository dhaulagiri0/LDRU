...
N_VALUES=(4 6 8 12)
SEQUENCE_LENGTHS=(40 60 100 150)
...
RESULTS_DIR="${ARCHITECTURE}_d_n_length_exps"
mkdir -p "$RESULTS_DIR"
...
echo "Sequence Lengths: ${SEQUENCE_LENGTHS[*]}" | tee -a "$LOG_FILE"
...
total_experiments=$((${#N_VALUES[@]} * ${#SEEDS[@]} * ${#SEQUENCE_LENGTHS[@]}))
...
for n in "${N_VALUES[@]}"; do
    for seed in "${SEEDS[@]}"; do
        for seq_length in "${SEQUENCE_LENGTHS[@]}"; do
            ...
            echo "[$current_experiment/$total_experiments] Running: Task=d_n, n=$n, Seed=$seed, SeqLength=$seq_length, LR=$lr, Steps=$steps" | tee -a "$LOG_FILE"
            
            # Create output filenames with n and sequence length parameters
            output_file="$RESULTS_DIR/d_n_n${n}_seq${seq_length}_seed${seed}_${ARCHITECTURE}.out"
            error_file="$RESULTS_DIR/d_n_n${n}_seq${seq_length}_seed${seed}_${ARCHITECTURE}.err"
...
echo "Sequence Lengths tested: ${SEQUENCE_LENGTHS[*]}" >> "$summary_file"
echo "Seeds used: ${SEEDS[*]}" >> "$summary_file"
echo "" >> "$summary_file"

echo "Results by N value, Length and Seed:" >> "$summary_file"
echo "----------------------------" >> "$summary_file"
...
for n in "${N_VALUES[@]}"; do
    ...
    for seq_length in "${SEQUENCE_LENGTHS[@]}"; do
        echo "Sequence Length: $seq_length" >> "$summary_file"

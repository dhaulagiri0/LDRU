...
folder_name="P_${pref_length}_${num_symbols}"
task_name="P_${pref_length}_${num_symbols}_LSTM"
...
for seed in {3..9}; do
    ...
    python -m ldru.training.example \
        ...
        --lr 1e-3 \
        --dropout_prob 0.0 \
        --eval_data_path "ldru/test_data/${folder_name}" \
        > "${task_name}/lstm_${task_name}_s${seed}.out" 2> "${task_name}/lstm_${task_name}_s${seed}.err"
    echo "Completed LSTM ${task_name} experiment with seed $seed."
done

"""Training loop for LDRU experiments."""
...
from ldru.tasks import task as task_lib
from ldru.training import curriculum as curriculum_lib
from ldru.training import range_evaluation
...
optimizer_from_string = {"adam": optax.adam, "amsgrad": optax.amsgrad,
                         "radam": optax.radam, "sgd": optax.sgd}
...
class ClassicTrainingParams:
  ...
  eval_data_path: Optional[str] = None  # Path to pre-generated evaluation data.
  
  use_tensorboard: bool = True # log to tensorboard
  validate: bool = False  # Whether to validate the model during training.
  
  if not use_tensorboard:
     logging.warning("Tensorboard logging is disabled. "
                     "Set `use_tensorboard=True` in the training parameters to enable it.")
...
class TrainingWorker:
  ...
  def __init__(self,
    ...
    self.validate = training_params.validate
    exp_name = training_params.task.__class__.__name__
    # Rename experiment for better readability
    if exp_name == "D_n":
        exp_name = f"D_{training_params.task.n}"
    elif exp_name == "NPrefixSymbols":
        exp_name = f"P_{training_params.task.prefix_length}_{training_params.task.num_symbols}"
    ...
    if self._log_tensorboard:
        ...
        while os.path.exists(f"tensorboard/{exp_name}"):
        ...
        self._writer = SummaryWriter(log_dir=f"tensorboard/{self.name}")
  ...
  def run(
    ...
    # Activate linear warm-up if warmup_frac is not 0.0
    ...
    # To initialize parameters, we need a dummy batch.
    dummy_batch = task.sample_batch(
        next(rng_seq), length=training_params.max_range_test_length+1, batch_size=4)
    # Sample validation batch (unused if validate=False)
    eval_batch = task.sample_batch(
        next(rng_seq), length=training_params.max_range_test_length, batch_size=1024)

    eval_sequence = eval_batch["input"].argmax(axis=-1)
    ...
    try:
      for step in steps:
        ...
        params, opt_state, (
            train_loss, train_metrics, train_accuracy, predictions, grads) = _update_parameters(
                params=params,
    ...
    if training_params.compute_full_range_test:
      eval_params = range_evaluation.EvaluationParams(
          ...
          data_file_path=training_params.eval_data_path,
      )
      # 
      if self._training_params.eval_data_path is not None:
        logging.info("Evaluating on pre-generated data file: %s", self._training_params.eval_data_path)
        eval_results = range_evaluation.range_evaluation_file(
            eval_params, use_tqdm=False, writer=self._writer if self._log_tensorboard else None)
      else:
        logging.info("Evaluating on sampled data.")
        eval_results = range_evaluation.range_evaluation_sample(
            eval_params, use_tqdm=False, writer=self._writer if self._log_tensorboard else None)

...
class Tomita4(task.GeneralizationTask):
    """A task to generate binary strings with Tomita 4 labels. A tomita 4 string is a binary sequence with no "000".
    Therefore any sequence with length 1 or 2 is tomita 4.
    For len > 2, we generate half positive examples and half negative examples.
    ...
    def __init__(self):
          ...
          self.num_states = 4  # q0 through q3
          ...
          # q0 transitions
          transition_matrix = transition_matrix.at[0, 0].set(1)  # q0 --0--> q1
          transition_matrix = transition_matrix.at[0, 1].set(0)  # q0 --1--> q0
          
          # q1 transitions
          transition_matrix = transition_matrix.at[1, 0].set(2)  # q1 --0--> q2
          transition_matrix = transition_matrix.at[1, 1].set(0)  # q1 --1--> q0
          
          # q2 transitions
          transition_matrix = transition_matrix.at[2, 0].set(3)  # q2 --0--> q3
          transition_matrix = transition_matrix.at[2, 1].set(0)  # q2 --1--> q0
          
          # q3 transitions (dead state)
          transition_matrix = transition_matrix.at[3, 0].set(3)  # q3 --0--> q3
          transition_matrix = transition_matrix.at[3, 1].set(3)  # q3 --1--> q3
          
          # Define accepting states (q0, q1, q2 are accepting)
          accepting_states = jnp.array([True, True, True, False])
    ...
    def belongs_to_lang(self, seq):
        ...
        """
        return self.dfa.run_batch(seq)[1]
    ...
    def generate_positives(self, rng, batch_size, length):
        ...
        oversample_factor = 1  # Adjust based on expected rejection rate
        ...
        def generate_sequence(sub_rng, length):
            ...
            def get_valid_options(state, sub_rng):
                return jax.lax.switch(state, [
                    lambda key: jax.random.choice(key, jnp.array([0, 1])),  # State 0: can use 0 or 1
                    lambda key: jax.random.choice(key, jnp.array([0, 1])),  # State 1: can use 0 or 1
                    lambda _: 1,     # State 2: can only use 1 to ensure no 000
                    lambda key: jax.random.choice(key, jnp.array([0, 1])),     
                ], sub_rng)
    ...
    def generate_negatives(self, rng, batch_size, length):
        ...
        """
        # Oversample by a factor to account for filtering
        oversample_factor = 3  # Adjust based on expected rejection rate
        oversampled_size = int(batch_size * oversample_factor)
    ...
    def sample_batch(self, rng, batch_size, length):
        ...
        if length < 3:
            inputs = jax.random.randint(rng, (batch_size, length), 0, 2)
...
if __name__ == '__main__':
  ...
  for i in range(1, 41, 1):
    sample = tomita4.sample_batch(jax.random.PRNGKey(i), 16, i)

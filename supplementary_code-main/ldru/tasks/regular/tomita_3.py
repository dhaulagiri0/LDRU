...
class Tomita3(task.GeneralizationTask):
    def __init__(self):
        ...
        self.num_states = 5  # q0 through q4
        ...
        # q1 transitions
        transition_matrix = transition_matrix.at[1, 0].set(3)  # q1 --0--> q3
        transition_matrix = transition_matrix.at[1, 1].set(0)  # q1 --1--> q0
        
        # q2 transitions
        transition_matrix = transition_matrix.at[2, 0].set(3)  # q2 --0--> q3
        transition_matrix = transition_matrix.at[2, 1].set(1)  # q2 --1--> q1
        
        # q3 transitions
        transition_matrix = transition_matrix.at[3, 0].set(2)  # q3 --0--> q2
        transition_matrix = transition_matrix.at[3, 1].set(4)  # q3 --1--> q4
        
        # q4 transitions (dead state)
        transition_matrix = transition_matrix.at[4, 0].set(4)  # q4 --0--> q4
        transition_matrix = transition_matrix.at[4, 1].set(4)  # q4 --1--> q4
        
        # Define accepting states
        accepting_states = jnp.array([True, True, True, False, False])
        ...
        def generate_sequence(sub_rng, length):
            ...
            def get_valid_options(state, sub_rng):
                return jax.lax.switch(state, [
                    ...
                    lambda key: jax.random.choice(key, jnp.array([0, 1])),     # State 2: can only use 1
                    lambda _: 0,     # State 3: can only use 0
    ...
    def generate_negatives(self, rng, batch_size, length):
        ...
        """
        # Oversample by a factor to account for filtering
        oversample_factor = 2.5  # Adjust based on expected rejection rate
        oversampled_size = int(batch_size * oversample_factor)
        ...
        # Create a mask for sequences not ending in state 3 (i.e., ending in 0, 1, or 2)
        is_rejected = jnp.logical_or(final_states == 3, final_states == 4)
    ...
    def sample_batch(self, rng, batch_size, length):
        ...
        if length == 1:
            inputs = jax.random.randint(rng, (batch_size, 1), 0, 2)
    ...
    def output_size(self) -> int:
        ...
        return 2  # Two classes: accepted (1) and rejected (0)
...
if __name__ == '__main__':
  ...
  for i in range(1, 41, 1):
    sample = tomita3.sample_batch(jax.random.PRNGKey(i), 16, i)

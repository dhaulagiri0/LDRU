...
class Tomita5(task.GeneralizationTask):
    ...
    def __init__(self):
          ...
          # q0 transitions
          transition_matrix = transition_matrix.at[0, 0].set(3)  # q0 --0--> q3
          transition_matrix = transition_matrix.at[0, 1].set(1)  # q0 --1--> q2
          ...
          # q2 transitions
          transition_matrix = transition_matrix.at[2, 0].set(1)  # q2 --0--> q3
          transition_matrix = transition_matrix.at[2, 1].set(3)  # q2 --1--> q0
          
          # q3 transitions (dead state)
          transition_matrix = transition_matrix.at[3, 0].set(0)  # q3 --0--> q3
          transition_matrix = transition_matrix.at[3, 1].set(2)  # q3 --1--> q3
          
          # Define accepting states (q0, q1, q2 are accepting)
          accepting_states = jnp.array([True, False, False, False])
    ...
    def generate_negatives(self, rng, batch_size, length):
        ...
        is_rejected = jnp.logical_or(jnp.logical_or(final_states == 1, final_states == 2), final_states == 3)
...
if __name__ == '__main__':
  ...
  for i in range(1, 41, 1):
    sample = tomita5.sample_batch(jax.random.PRNGKey(i), 16, i)
